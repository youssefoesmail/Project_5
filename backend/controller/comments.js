...
const getCommentByPostId = (req, res) => {
  const { id } = req.params;
  const query = `SELECT comment_posts.comment, comment_posts.post_id, users.firstName, users.photo, comment_posts.commenter,comment_posts.id, comment_posts.is_deleted
  FROM comment_posts
